 ...
 *  into a floating popover docked over the top of that field instead of a
 *  block sitting below it: it shows while the field has focus and there are
 *  suggestions, and hides on blur. The field's container must be
 *  `position: relative` for the overlay to anchor correctly. */
class SuggestionBar {
    constructor(container, { onSelect, anchorTo } = {}) {
        ...
        if (this.anchorTo) {
            ...
            this.container.classList.add(
                'absolute', 'z-30', 'top-1', 'left-1', 'right-1',
                'max-h-[calc(100%-0.5rem)]', 'overflow-y-auto',
    ...
    render(suggestions) {
        ...
        this.container.innerHTML = this.suggestions.map((s, i) => {
            ...
            const char = s.pron.char ? s.pron.char() : (s.pron.simp || '?');
            ...
            return `
                <button type="button" data-suggestion-index="${i}"
                    ...
                    <span class="hanzi text-base font-semibold leading-none">${char}</span>
